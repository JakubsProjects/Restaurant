...
int main(int argc, char **argv)
{
	...
	for (x = 0; x < argc; x++) {
		if (strcmp(argv[x], "-d") == 0) {    //If the flag is -d 
			DEBUGMODE = 1;
			fprintf(stderr, "The flag '-d' was passed\n");    //Then activate debugging mode
	...
	while ((ch = getNextNWSChar()) != EOF)
	{
		...
		{
			printCommands(); //Function to print the commands we can use in program
		}
		else if ('a' == ch)
		{
			doAdd(&head, in_restaurant, DEBUGMODE); //Add to the list, (people in restaurant)
		}
		else if ('c' == ch)
		{
			doCallAhead(&head, in_callahead, DEBUGMODE);//Add to the list, (people on call)
		}
		else if ('w' == ch)
		{
			doWaiting(&head, DEBUGMODE); //Change status from on-call to waiting in restaur.
		}
		else if ('r' == ch)
		{
			doRetrieve(&head, DEBUGMODE);//Sit people down in restuarant
		}
		else if ('l' == ch)
		{
			doList(&head, DEBUGMODE); //Display amount people are in front of a specific group
		}
		else if ('d' == ch)
		{
			doDisplay(head, DEBUGMODE); //Display people in front of a specific group
		}
		else
		{
			printf("%c - in not a valid command\n", ch); //Print error
			printf("For a list of valid commands, type ?\n");  //Suggetsion to see commands
			clearToEoln();
		}
